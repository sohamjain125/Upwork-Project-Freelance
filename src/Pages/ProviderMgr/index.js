...
export default function ProviderMgr(props) {
    ...
    const  header = ["Company Code","Company Name","Tax identification number","Phone","Email","Address","Country","Code Postal","Fax","Observation"]
    ...
    const getDataUser = async () => {
        ...
        let res = null;
        try {
            let response = await fetch(`${process.env.REACT_APP_API_URL}/getProviders?limit=${limit}&offset=${offset}`);
            res = await response.json();
            const body = await Promise.all(res.map(obj => {
                const temp = [];
                temp.push(obj.companyCode)
                temp.push(obj.companyName)
                temp.push(obj.taxIdentificationNumber)
                temp.push(obj.phone)
                temp.push(obj.email)
            ...
            <div>
                ...
                <main>
                    ...
                    <Actions 
                        ...
                        data={data} 
                        onAdd={Events.addProviders}
                        onEdit={Events.editProviders}
                        onDelete={Events.deleteProviders}
                        onConfirmAdd={Events.confirmAddProviders}
                        onConfirmEdit={Events.confirmEditProviders}
