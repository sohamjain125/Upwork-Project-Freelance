...
export default function Products(props) {
    ...
    const  header = ["CodeProduct","Category","Description","Origin","PriceHT","RateVAT","discount"]
    ...
    const getDataUser = async () => {
        ...
        let res = null;
        try {
            let response = await fetch(`${process.env.REACT_APP_API_URL}/getProducts?limit=${limit}&offset=${offset}`);
            res = await response.json();
            const body = await Promise.all(res.map(obj => {
                const temp = [];
                temp.push(obj.codeProduct)
                temp.push(obj.category)
                temp.push(obj.description)
                temp.push(obj.origin)
                temp.push(obj.priceHT)
                temp.push(obj.rateVAT)
                temp.push(obj.discount)
                return temp;
            }));
            setdata({
                header: header,
    ...
    return <div>
        ...
        {isLoading ||
            <div>
                ...
                <Sidebar />
                <main>
                    <Card title="Products" text="From this table you can manage the list of client:" />
                    <Actions 
                        visibility={!tableLoading} 
                        data={data} 
                        onAdd={Events.addProducts}
                        onEdit={Events.editProducts}
                        onDelete={Events.deleteProducts}
                        onConfirmAdd={Events.confirmAddProducts}
                        onConfirmEdit={Events.confirmEditProducts}
