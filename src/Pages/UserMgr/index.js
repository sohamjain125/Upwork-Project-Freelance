...
export default function UserMgr(props) {
    ...
    const  header = ["username","password","createdAt","lastLogin"]
    ...
    const getDataUser = async () => {
        ...
        let res = null;
        try {
            let response = await fetch(`${process.env.REACT_APP_API_URL}/getUser?limit=${limit}&offset=${offset}`);
            res = await response.json();
            const body = await Promise.all(res.map(obj => {
                const temp = [];
                temp.push(obj.username)
                temp.push(obj.password)
                temp.push(obj.createAt)
                temp.push(obj.lastLogin)
                return temp;
            }));
            setdata({
                header: header,
            ...
            <div>
                ...
                <main>
                    ...
                    <Actions 
                        ...
                        data={data} 
                        onAdd={Events.addUser}
                        onEdit={Events.editUser}
                        onDelete={Events.deleteUser}
                        onConfirmAdd={Events.confirmAddUser}
                        onConfirmEdit={Events.confirmEditUser}
