...
export default function WorkersMgr(props) {
    ...
    const header = ["Client Code","First Name","Last Name","Phone","Email","Address","Country","Payinf Frequency","Daily income","Worked Days","Currently Accumulated","Total Salary","Taken Advance","Net to Pay"]
    ...
    const getDataUser = async () => {
        ...
        let res = null;
        try {
            let response = await fetch(`${process.env.REACT_APP_API_URL}/getWorkers?limit=${limit}&offset=${offset}`);
            ...
            const body = await Promise.all(res.map(obj => {
                ...
                temp.push(obj.payedBy)
                temp.push(obj.unitCost)
                temp.push(obj.totalWorked)
                temp.push(obj.accumulatedSalary)
                temp.push(obj.totalSalary)
                temp.push(obj.salaryAdvance)
                temp.push(obj.salaryNetToPay)
                return temp;
            }));
            setdata({
                header: header,
            ...
            <div>
                ...
                <main>
                    ...
                    <Actions 
                        ...
                        data={data} 
                        onAdd={Events.addWorkers}
                        onEdit={Events.editWorkers}
                        onDelete={Events.deleteWorkers}
                        onConfirmAdd={Events.confirmAddWorkers}
                        onConfirmEdit={Events.confirmEditWorkers}
