...
export default function StockManagement(props) {
    ...
    const  header = ["Movement Date","Product Code","Quantity"]
    ...
    const getDataUser = async () => {
        ...
        let res = null;
        try {
            let response = await fetch(`${process.env.REACT_APP_API_URL}/getStock?limit=${limit}&offset=${offset}`);
            res = await response.json();
            const body = await Promise.all(res.map(obj => {
                const temp = [];
                temp.push(obj.movementDate)
            ...
            <div>
                ...
                <main>
                    ...
                    <Actions 
                        ...
                        data={data} 
                        onAdd={Events.addStock}
                        onEdit={Events.editStock}
                        onDelete={Events.deleteStock}
                        onConfirmAdd={Events.confirmAddStock}
                        onConfirmEdit={Events.confirmEditStock}
